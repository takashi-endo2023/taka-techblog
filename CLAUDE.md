...
- **新記事を書いたら、記事タイプに応じてアフィリを配置する**（`docs/affiliate-strategy.md` の「新記事作成時のアフィリ選定ルール」表で機械判定）。技術解説→Amazon関連書、テックリード/組織→GEEKLY、未経験/学習→Udemy、純粋な体験談→入れない。記事末尾の「## この記事を読んだ方へ」セクションに置けば sync:zenn が Zenn 側から自動除去する
...
|---|---|---|
| `status.md` | **現在地・当面の唯一KPI・今週やること(3つ)・振り返り**（実行ダッシュボード）| **毎日〜毎週** |
| `post-log.md` | **日次の軽量投稿ログ（1日1行・絡み件数・フロア）**（立ち上げ初期の継続管理用）| **毎日** |
| `career-roadmap.md` | 長期キャリア（38→45の現実版フェーズ）・勤務先の状況と分岐シナリオ | 方向に迷ったとき |
