...
pubDate: "2026-02-15"
tags: ["GitHub Actions", "AWS", "OIDC", "DevOps", "セキュリティ"]
---
...
このブログでは S3 + CloudFront が必要なので、最小権限の原則で絞ります：
...
`id-token: write` の permissions 設定が必要な点がハマりポイントです。これがないとOIDCトークンが発行されず、認証エラーになります。
