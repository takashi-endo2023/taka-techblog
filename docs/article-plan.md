# 記事計画・アイデアメモ

最終更新: 2026-05-27

---

## 現在の記事一覧（カテゴリ別）

### キャリア・組織
| pubDate | タイトル | ファイル |
|---|---|---|
| 2023-06-15 | 島流しの先にエンジニアがあった——30歳からの転身の現実 | career-change-to-engineer.md |
| 2024-03-26 | 同僚が部下になった日——テックリード就任、最初の90日の現実 | techLead-first-90-days.md |
| 2024-06-30 | 外部ベンダーから内製化した3年間 — 年間480万円の固定費をゼロにするまで | naisei-kansei.md |
| 2024-07-13 | 直したいのに直せない——技術的負債と向き合う3年間 | technical-debt-strategy.md |
| 2024-07-27 | 未経験エンジニア2名をコーチング型で育てた方法 | coaching-junior-engineers.md |
| 2024-08-09 | コードより先に要件を言語化させる——会話が成立しない部下への対処 | junior-engineer-spec-before-code.md |
| 2024-10-16 | テックリードと係長を兼任するプレイングマネージャーの現実 | playing-manager-reality.md |
| 2024-10-30 | 3人で回すということ——少人数チームを機能させるための工夫 | small-elite-team-design.md |
| 2024-11-13 | 目標は私が暇になること——コードレビューを3年一人でやってきた話 | code-review-culture.md |
| 2024-11-26 | 誰も採用をやらないから、自分でやることにした | engineer-hiring-lessons.md |
| 2025-08-12 | 「理屈が通れば動く人」との3年——経営層への技術翻訳の実際 | communicating-it-value-to-executives.md |
| 2025-09-08 | AIがなかったら死んでた——一人で全部背負うテックリードの現実 | solo-techlead-ai-survival.md |
| 2025-09-21 | みんなの安定のために仮面を被った——テックリード3年間の全記録 | techlead-mask-3years.md |

### JavaScript / TypeScript
| pubDate | タイトル | ファイル |
|---|---|---|
| 2023-06-29 | ES6+の新構文を実務でどう使うか——JavaScript本格入門で整理した基礎知識 | javascript-es6-modern-syntax.mdx |
| 2023-07-12 | JavaScriptのスコープとthisで詰まったこと——本格入門で理解が深まった概念 | javascript-scope-closure.mdx |
| 2023-07-26 | JavaScriptの非同期処理を整理する——コールバック・Promise・async/awaitの使い分け | javascript-async-promise.mdx |
| 2023-08-08 | JavaScriptのArray高階関数を実務で使いこなす——map・filter・reduceの使い分け | javascript-array-methods.mdx |
| 2023-08-22 | JavaScriptのクラスとモジュールを理解する——TypeScriptに活きるOOPの基礎 | javascript-class-module.mdx |
| 2023-09-05 | ESLint・Jest・Viteを最初に整備する——JavaScriptプロジェクトの開発環境構築 | javascript-dev-tools-eslint-jest.mdx |
| 2025-10-18 | TypeScript Utility Types実践ガイド——Partial・Pick・Omitを実務でどう使うか | typescript-utility-types-practical.mdx |
| 2025-12-25 | 改訂3版 JavaScript本格入門——100万部超えロングセラーを独学エンジニアが読んだ正直な評価 | javascript-honkaku-nyumon-review.mdx |
| 2026-01-08 | JavaScriptのMap・Setを使いこなす——Arrayとの使い分けを実務目線で整理する | javascript-map-set-practical.mdx |
| 2026-01-22 | Fetch APIで非同期通信をマスターする——GET・POST・エラーハンドリング・React連携まで | javascript-fetch-api-practical.mdx |
| 2026-02-04 | JavaScriptの正規表現入門——実務でよく使うパターンをコード例で整理する | javascript-regexp-practical.mdx |

### React / Next.js / フロントエンド設計
| pubDate | タイトル | ファイル |
|---|---|---|
| 2023-11-25 | Atomic Designを実務で使って気づいたこと——設計の共通言語はチームを助ける | atomic-design-in-practice.mdx |
| 2023-12-09 | Next.jsのSSR・SSG・ISRを実務でどう使い分けるか | nextjs-rendering-deep-dive.mdx |
| 2023-12-22 | StorybookをReactチームに導入して見えてきたこと——メリットと継続の難しさ | storybook-team-adoption.mdx |
| 2024-01-05 | Next.jsでSEOを実装するときに押さえるべきこと——メタデータからJSON-LDまで | nextjs-seo-implementation.mdx |
| 2024-01-19 | フロントエンドのテスト戦略——「全部書く」をやめてから品質が上がった話 | frontend-testing-practical-approach.mdx |
| 2024-02-01 | TypeScriptとReact/Next.js実践本を未経験エンジニアが読んだ正直な感想 | typescript-react-nextjs-beginner-review.mdx |
| 2024-02-15 | TypeScriptとReact/Next.js実践本を読んで実務で使い続けた話——テックリード視点の正直な評価 | typescript-react-nextjs-book-review.mdx |
| 2025-08-25 | React×Next.jsの選択基準を実務から考える —SPA・SSR・SSGをどう使い分けるか | react-nextjs-selection.md |
| 2025-11-01 | styled-components vs CSS Modules——Next.jsプロジェクトでCSS設計を選ぶ基準 | css-in-js-styled-components-vs-modules.mdx |
| 2025-11-15 | Next.jsのデプロイ先を選ぶ——Vercel・AWS・Herokuを比較して学んだこと | nextjs-deployment-strategy.mdx |
| 2025-11-28 | フロントエンドエンジニアが最低限知っておくべきアクセシビリティ入門 | frontend-accessibility-intro.mdx |
| 2025-12-12 | StripeとNext.jsで決済機能を実装する——Checkout・Webhook・本番対応まで | nextjs-stripe-payment.mdx |

### インフラ / Linux / Web基礎
| pubDate | タイトル | ファイル |
|---|---|---|
| 2023-09-18 | Linux運用で最低限知るべきこと——ターミナル作業を怖くなくするための基礎知識 | linux-server-operations-basics.mdx |
| 2023-10-02 | フロントエンドエンジニアがサーバ/インフラを学んで気づいたこと——見えていなかった仕組み | server-infra-basics-for-frontend.mdx |
| 2023-10-15 | HTTPを正しく理解する——メソッド・ステータスコード・ヘッダーの実務知識 | http-methods-status-codes.mdx |
| 2023-10-29 | 望ましいURIとは何か——Webを支える技術で学んだURL設計のベストプラクティス | uri-design-best-practices.mdx |
| 2023-11-12 | REST APIの設計原則を学び直す——URLの考え方から冪等性まで | rest-api-design-web-basics.mdx |
| 2024-05-20 | Webサービスのパフォーマンスチューニング入門——計測・キャッシュ・DBが三本柱 | web-performance-tuning-basics.mdx |
| 2024-06-02 | 高可用性設計の基本——SPOF排除とフェイルオーバーで「止まらないサービス」を作る | high-availability-design-spof.mdx |
| 2024-06-16 | Docker開発環境を標準化してチームの「動かない」を減らす | docker-dev-environment.md |
| 2026-02-18 | Webを支える技術——HTTPとRESTを根本から理解したいエンジニアへの正直な評価 | web-technology-book-review.mdx |
| 2026-03-03 | WebアーキテクチャとRESTが生まれた理由——設計思想を歴史から理解する | web-architecture-rest-history.mdx |
| 2026-03-17 | HTTPキャッシュ完全ガイド——Cache-Control・ETag・CDN連携を実務で使いこなす | http-cache-guide.mdx |
| 2026-03-31 | REST APIの書き込み設計——冪等性・トランザクション・楽観的ロックの実装パターン | rest-api-write-design.mdx |
| 2026-04-13 | サーバ/インフラを支える技術——止まらないサービスを作るための設計思想を学んだ正直な評価 | server-infra-book-review.mdx |
| 2026-04-27 | Linuxロードアベレージを正確に理解する——sar・vmstat・psで障害の原因を特定する | linux-load-average-deep-dive.mdx |
| 2026-05-10 | MySQLレプリケーション設計——障害復旧・読み取り分散・RDS Multi-AZとの比較 | mysql-replication-design.mdx |
| 2026-05-24 | リバースプロキシの役割と設計——Nginx・CDN・キャッシュ戦略 | reverse-proxy-cache-design.mdx |

### AWS / IaC / CI/CD
| pubDate | タイトル | ファイル |
|---|---|---|
| 2024-02-28 | AWSのIAM設計を正しく理解する——最小権限の原則と実務での落とし穴 | aws-iam-least-privilege.mdx |
| 2024-03-13 | AWS VPCのサブネット設計を理解する——パブリック・プライベートの分け方とセキュリティ設定 | aws-vpc-subnet-design.mdx |
| 2024-04-09 | CloudWatchでAWSを監視する——メトリクス・アラーム・ログの実務パターン | aws-cloudwatch-monitoring.mdx |
| 2024-04-23 | EC2・Lambda・ECSをどう使い分けるか——AWSコンピューティングの選択基準 | aws-compute-ec2-lambda-ecs.mdx |
| 2024-05-06 | RDSとDynamoDBの使い分け——データ構造とアクセスパターンで選ぶ基準 | aws-rds-dynamodb-choice.mdx |
| 2024-09-19 | mainブランチを捨てた——リリース1時間前のgit修羅場 | git-chaos-1hour-before-release.md |
| 2025-03-15 | AstroとAWS CDK + CloudFrontで技術ブログを構築した話 | astro-aws-cdk-cloudfront-blog.md |
| 2025-03-29 | AWS CDKを初めて使って詰まった5つのこと | aws-cdk-first-pitfalls.md |
| 2025-04-11 | AstroサイトのS3+CloudFrontキャッシュ戦略：静的サイトを最速にする | astro-s3-cache-strategy.md |
| 2025-04-25 | S3 + CloudFrontで静的サイトを本番運用する——このブログを構築して学んだこと | aws-s3-cloudfront-static-site.mdx |
| 2025-05-08 | GitHub ActionsとAWS OIDCでキーレスデプロイを実現する | github-actions-oidc-aws.md |
| 2025-05-22 | GitHub Actions + AWSでCI-CDを構築する——OIDCでアクセスキーなしの安全なデプロイ | aws-devops-github-actions.mdx |
| 2025-06-05 | GitLab CI-CDを実務で運用するときの設計パターン | gitlab-cicd-patterns.md |

### NestJS / バックエンド
| pubDate | タイトル | ファイル |
|---|---|---|
| 2025-02-16 | NestJSの治験CRMを引き継いで学んだバックエンド設計 | nestjs-crm-design.md |
| 2025-03-01 | NestJS + LangChainで治験CRMにAI機能を追加した構成メモ | nestjs-langchain-crm-architecture.md |

### AI / LangChain / Claude Code
| pubDate | タイトル | ファイル | 備考 |
|---|---|---|---|
| 2024-12-23 | LangChain.js 2025年の現状：実務で使って感じたこと | langchain-js-2025.md | LangChain概要・実務評価 |
| 2025-01-20 | RAGをやめてツールベースに切り替えた話——LangChain.jsとCRMの相性 | langchain-nestjs-ai-integration.md | 実装詳細 |
| 2025-02-02 | TypeScriptで型安全なLangChainアプリを作る —LangChain.jsの型定義と実装パターン | typescript-langchain-type-safe.md | 型設計 |
| 2025-06-18 | Cursor実務レビュー2025：Claude Codeと使い分けて気づいたこと | cursor-review-2025.md | Cursor vs Claude Code |
| 2025-07-02 | Claude Codeを実務導入して開発フローが変わった話 | claude-code-workflow.md | 導入・効果測定済み |
| 2025-07-15 | Claude Codeを設計の壁打ち相手として使う：アーキテクチャ相談の実践 | claude-code-as-design-partner.md | 設計相談ユースケース済み |
| 2025-07-29 | AIで議事録を構造化する：「要約」では使えない理由とプロンプト設計 | ai-meeting-notes-structured.md | 業務効率化ユースケース |
| 2025-10-05 | AIを社内に広めたら、本部長がモンスターになった話 | ai-team-adoption-monster.md | 組織導入の現実 |

### 医療IT / 治験 / 規制対応
| pubDate | タイトル | ファイル | 備考 |
|---|---|---|---|
| 2024-08-23 | CSV（コンピュータ化システムバリデーション）とは何か — 医療ITエンジニア目線で解説 | csv-validation-guide.md | CSV概要・バリデーション工程済み |
| 2024-09-06 | 製薬メーカーのシステム監査に技術者として対応した話 | system-audit-experience.md | 監査対応実体験済み |
| 2024-10-03 | 引き継いだリリースフローが、監査で初めて意味を持った | release-flow-regulated-industry.md | 変更管理プロセス済み |
| 2025-01-06 | 治験データが増えるにつれてシステムが遅くなった——パフォーマンス改善の実記録 | clinical-trial-system-performance.md | N+1・パフォーマンス済み |

### その他
| pubDate | タイトル | ファイル |
|---|---|---|
| 2024-12-10 | グループ会社のIT環境をゼロから整備した話——MakeShopとGA4と諸々 | makeshop-technical-notes.md |

---

## 📝 記事アイデア（未執筆）

### 🤖 AI・開発効率化（優先度: 高）

#### 1. AI×業務フロー効率化：社内コンサル的なアプローチ
- **想定ファイル**: `ai-business-process-consulting.md`
- **概要**: エンジニアがAIを社内に広める立場になったとき。技術者だけの話ではなく、非エンジニアの業務（議事録・報告書・メール下書き・データ整理）にどうAIを届けるか。社内ワークショップ・ハンズオンの設計
- **自分の切り口**: 医療系で「個人情報をAIに入力していいか？」という懸念への向き合い方

#### 2. 結合試験をボタン1つで完結させる：CI自動化の設計
- **想定ファイル**: `integration-test-automation.md`
- **概要**: 手動で行っていた結合試験フローをCI/CDに組み込む。テストシナリオの設計・Playwright等E2Eツール選定・ステージング環境での自動実行・レポート生成まで
- **自分の切り口**: 治験システムでの「証跡が残る自動テスト」。規制産業では「テストが実行された証拠」そのものが求められる
- **関連記事**: `frontend-testing-practical-approach.mdx`, `github-actions-oidc-aws.md`

#### 3. PRマージ時の品質ゲート：マージ前に問題を自動検知する仕組み
- **想定ファイル**: `pr-quality-gate-automation.md`
- **概要**: GitHub ActionsでPRマージ前に自動でチェックする仕組み。型チェック・lint・テスト・セキュリティスキャン（dependabot/trivy）・カバレッジしきい値。Branch Protectionルールとの連携
- **自分の切り口**: 少数精鋭チームでレビュワーが1人しかいないとき、機械的なチェックに頼ることで人間のレビューを設計判断に集中させた体験

#### 4. Claude Code × チーム開発：1人で使うツールからチームへ広げるとき
- **想定ファイル**: `claude-code-team-adoption.md`
- **概要**: Claude Codeを自分だけが使っている状態から、チームに展開するときの課題。コスト・セキュリティポリシー・プロンプト共有・使いどころのすり合わせ
- **自分の切り口**: 規制産業でのAIツール導入承認プロセス。「このデータをAIに渡していいか」の判断基準を作った話

#### 5. AIを使ったコードレビュー支援：人間のレビューと何が違うか
- **想定ファイル**: `ai-code-review-assistant.md`
- **概要**: AIに初回レビューをさせてから人間がレビューする2段階方式。AIが得意なこと（命名・重複・バグパターン）と人間が必要なこと（設計判断・仕様理解）の棲み分け
- **関連記事**: `code-review-culture.md`

---

### 🤖 社内AI展開シリーズ（現在進行形・体験談）

> **重要**: まだ全社展開は実現していない。「成功事例」ではなく「現在進行形・計画・設計」として書く。
> 現状＝システム部と本部長だけがClaude導入済み。非エンジニアへの展開はこれから。
> `ai-team-adoption-monster.md`（本部長モンスター・公開済み）の続編として内部リンクで繋ぐ。

#### 6. システム部と本部長で止まっている——社内AI展開の現在地 ✅執筆済み（2026-06-14公開予約）
- **ファイル**: `ai-rollout-stuck-state.md`
- **概要**: AIを全社に広めたいのに、システム部と本部長の2グループで止まっている現状の正直な記録。本部長は暴走、現場には届かないギャップ。なぜ広がらないかの壁
- **切り口**: 結論を出さない現在進行形。多くの組織が抱える「一部だけ先行して止まる」問題への共感
- **関連記事**: `ai-team-adoption-monster.md`

#### 7. 非エンジニアにAIをどう教えるか——社内AIコンサルとして考えていること
- **想定ファイル**: `teaching-ai-to-non-engineers.md`
- **概要**: 「AIとは」をどう噛み砕くか、「Claudeでこの業務が減る」をどう伝えるか。教える側の視点・フレームワーク。#1（社内コンサル的アプローチ）と統合してもよい
- **切り口**: 技術を非技術者に翻訳する力。計画段階でも書ける（考え方が主題）

#### 8. 個人情報をAIに入れていいのか——医療系でAI展開する難しさ
- **想定ファイル**: `medical-ai-adoption-barriers.md`
- **概要**: 規制産業ならではのAI導入の壁。個人情報・コンプライアンス・「このデータをAIに渡していいか」の判断
- **切り口**: 医療IT × AI の独自性。ニッチだが深く刺さる。#4（Claude Codeチーム展開）の規制視点と連携

---

### 🛠️ AI駆動開発フロー標準化シリーズ（計画中・独自メソッド）

> **重要**: まだチーム展開は計画段階。「設計した話」として書く（設計自体は実在＝authentic）。
> `code-review-culture.md`（公開済み・自動化ステージ1-4を"妄想"として記述）の**実装編**。
> 元資料: `~/Documents/claudeアーティファクト/システム開発部用/`（3層設計・スタック別キット・移行ロードマップ）
> このタイプ（独自メソッド × Claude Code × 持論）は汎用技術解説と違い Zenn で強い。

#### 9. 理解度セルフゲート——AIに説明させず「質問だけ」生成させる ✅執筆済み（2026-06-21公開予約）
- **ファイル**: `ai-understanding-self-gate.md`
- **概要**: `/explain-check`。AI出力そのまま/意図を説明できないPRをlintで捕れない問題。「意図を聞く→答えられない→わかるまで考えて来い」を提出前セルフゲートに自動化。原理＝コードを説明させず質問だけ生成させる。答えられない箇所＝理解してない箇所
- **切り口**: 「AIは使え、ただし分かった上で出せ」を取り締まりでなく仕組みで担保。逆張り・持論性で拡散力
- **関連記事**: `code-review-culture.md`, `coaching-junior-engineers.md`

#### 10. レビュー基準を「頭」から「設定ファイル」へ——3層＋配布の設計
- **想定ファイル**: `review-standardization-3layers.md`
- **概要**: 爆速×平準化×品質維持。層1 CLAUDE.md（常時ロード）／層2 /review＋code-reviewer（観点固定）／層3 hooks＋CI（忘れても効く機械ゲート）／配布 プラグイン。対話レビューは人が忘れるとザル、CIに乗せて初めて守られる
- **切り口**: 「誰がやっても同じ観点・同じ重大度」を実現する設計思想（概論・シリーズの入口）

#### 11. CLAUDE.md に禁止事項を前倒しする——スタック別レビューキット
- **想定ファイル**: `claude-md-review-kit.md`
- **概要**: レビュー時間の大半は同じ指摘の繰り返し→禁止事項に前倒し。TS/React/Next/Nest別の禁止事項・観点・重大度（Blocker/Major/Minor/Nit）。AIスロップ検知（言い換えコメント・到達しない防御コード・実装のミラーなテスト・存在しないAPI）
- **切り口**: コピペで使える実践キット。実用性で読まれる

#### 12. AI駆動開発フローの段階移行——Phase 0〜5のロードマップ
- **想定ファイル**: `ai-dev-flow-migration-roadmap.md`
- **概要**: 個人設定(Phase0)→テスト土台(1)→マージ前ゲート(2)→CLAUDE.md＋CI自動レビュー(3)→人の再配置(4)→チーム配布(5)。一気にやらず段階で。改善の本質＝良かった指示を都度CLAUDE.md/observ点/テストへ「昇格」
- **切り口**: 組織・戦略視点。「資料は増やすより育てる」。テックリードの移行マネジメント

---

### 🏥 医療IT・治験・規制対応（優先度: 高）

#### 6. CSVドキュメントを1から作った話——バリデーションプロトコルの現実
- **想定ファイル**: `csv-documentation-reality.md`
- **概要**: csv-validation-guide.mdは「CSVとは何か」の解説。こちらは「実際に作ると何が大変か」の実体験。URS・FRS・IQ/OQ/PQ・トレーサビリティマトリクスを1から書いたときの工数・ハマりポイント・Excelで管理する限界
- **自分の切り口**: 「誰もやり方を教えてくれない」状態でGALP/GAMP5を読みながら手探りで作った話
- **注意**: csv-validation-guide.mdとは別の切り口（作業実体験）にする

#### 7. PPD・CROの品質監査に技術者として対応した話
- **想定ファイル**: `cro-audit-tech-response.md`
- **概要**: 製薬会社の委託先（CRO/PPDなど）が実施する品質監査でシステム担当として同席したときに何を聞かれるか。監査員の視点・よく指摘される項目・エビデンスとして準備すべき資料
- **自分の切り口**: system-audit-experience.mdとの違いは「製薬会社側から来た監査」vs「自社への監査対応」。具体的に聞かれた質問・詰められたポイント
- **注意**: system-audit-experience.mdはスポンサー監査の話。これはCRO・PPD等のグローバル品質組織からの監査

#### 8. 製薬会社からのシステム解凍——変更管理と「凍結」の現実
- **想定ファイル**: `system-freeze-change-control.md`
- **概要**: 治験稼働中はシステムを「凍結」し、変更には製薬会社の承認が必要になる。このプロセス（Change Request → Impact Assessment → Validation → Sponsor Approval → 解凍）の実体験。機能追加1つに3ヶ月かかったときの話
- **自分の切り口**: エンジニアの感覚では「バグ修正なのに3ヶ月待ち」の理不尽さと、規制の合理性の両方を理解した経験。スピードと安全のトレードオフを一番感じた場所

#### 9. 21 CFR Part 11 と電子署名——医療ITエンジニアが知るべき規制の最低限
- **想定ファイル**: `21cfr-part11-tech-guide.md`
- **概要**: FDA規制21 CFR Part 11が求める電子記録・電子署名の要件をエンジニア目線で整理。監査証跡・タイムスタンプ・署名者の一意識別・改ざん防止の実装レベルでの解釈
- **自分の切り口**: 「これを実装したらPart 11準拠か？」を製薬会社に確認するまでの不確実性。エンジニアには法的判断ができない部分をどう扱うか

---

### 🔧 開発フロー・CI/CD（優先度: 高）※今すぐ書ける

#### 12. GitLab CI × 結合試験自動化——「ボタン1つで試験が走る」を目指した設計記録
- **想定ファイル**: `gitlab-ci-integration-test-impl.md`
- **概要**: 手動結合試験をGitLab CIに組み込む実装記録。「人の手をかけない・誰がやっても同じ品質」をゴールにClaudeと設計壁打ちしながら構成を決めていったプロセス。stg/mainブランチへのマージ時に試験が自動実行される仕組みまで
- **自分の切り口**: 他部署の最優先依頼に割り込まれながらも「この自動化が終われば将来の依頼も早くなる」と主張して進めている現在進行形の話。Claudeをどう使って設計したか
- **関連記事**: `code-review-culture.md`, `gitlab-cicd-patterns.md`, `github-actions-oidc-aws.md`
- **備考**: 実装完了後に書く。進行中の話なので完成度が高くなる

#### 13. ブランチ戦略を作り直した話——mainが壊れた経験から逆算した設計
- **想定ファイル**: `git-branch-strategy-redesign.md`
- **概要**: リリース1時間前にmainブランチが壊れた修羅場の後、ブランチ運用をどう再設計したか。「stgを信頼の基点にする」発想・環境ごとのブランチの役割定義・マージルールの整備
- **自分の切り口**: git-chaos-1hour-before-release.mdの後日談。「直す」より「捨てて作り直す」判断を繰り返した経験から、安全なブランチ設計の原則を言語化
- **関連記事**: `git-chaos-1hour-before-release.md`, `release-flow-regulated-industry.md`

---

### 👥 チーム設計・マネジメント（優先度: 中）※今すぐ書ける

#### 14. 「やっといて」では動けない——タスク設計とチーム成長の仕組み
- **想定ファイル**: `task-design-for-junior-engineers.md`
- **概要**: 「やっといて」で任せると期限を守れない理由は方針がないから。「タスクを言語化→調査→作業」の3ステップで渡すフレームワークと、人に教えることで教える側も育つ連鎖の話
- **自分の切り口**: 期限を守れないメンバーを叱るのではなく、動き方のフレームを渡したら変わった実体験。後輩への指導を「無理矢理やらせた」ら先輩が成長し始めた話
- **関連記事**: `small-elite-team-design.md`, `coaching-junior-engineers.md`, `junior-engineer-spec-before-code.md`

#### 15. 委譲を恐れていた3年間——テックリードが手を離せなかった本当の理由
- **想定ファイル**: `why-techlead-cant-delegate.md`
- **概要**: 「自分がやった方が早い」「品質が落ちる」「責任が取れない」——委譲を妨げる3つの壁とその正体。完璧主義・孤立構造・手放すことへの恐怖を言語化し、段階的委譲で何が変わったかを書く
- **自分の切り口**: コードレビューもタスク管理も朝会の仕切りも、全部「渡してみたら意外となんとかなった」体験の積み重ね。「任せてみないとわからない」の連続
- **関連記事**: `code-review-culture.md`, `small-elite-team-design.md`, `solo-techlead-ai-survival.md`

---

### 🤖 Claude Code・AI活用（優先度: 中）

#### 16. このブログのCLAUDE.mdをどう設計したか——AIに自律的に動いてもらうルール設計
- **想定ファイル**: `claude-md-design-philosophy.md`
- **概要**: Claude Codeでブログ運営をするにあたって、CLAUDE.mdに何を書いて何を書かないかの設計思想。「コードを書かせる」ではなく「判断基準を渡す」という発想の転換。ルールが育っていく過程
- **自分の切り口**: 実際にこのブログで使っているCLAUDE.mdを題材に、どんな判断をClaudeに任せてどんな判断は人間が持つかを言語化。「任せる」設計の難しさ
- **関連記事**: `claude-code-as-design-partner.md`, `claude-code-workflow.md`

---

### ⚙️ 開発フロー・設計（優先度: 中）

#### 10. URI設計から始めるAPI設計——チームで合意できる設計書の作り方
- **想定ファイル**: `api-design-doc-template.md`
- **概要**: APIを作る前に「設計書を先に書く」フローの実践。URIリソース設計・メソッド・レスポンス形式・エラーコード定義を1枚のドキュメントにまとめてから実装するプロセス
- **関連記事**: `uri-design-best-practices.mdx`, `rest-api-design-web-basics.mdx`

#### 11. ステージング環境の設計——本番に近い環境を低コストで維持する
- **想定ファイル**: `staging-environment-design.md`
- **概要**: 本番と同等のステージング環境を作るときのトレードオフ。RDSのインスタンスサイズ・データのマスキング・外部連携のモック化。治験システムでの「テスト被験者データ」管理

---

## 📊 カテゴリ別現状と方針

| カテゴリ | 現在の記事数 | 方針 |
|---|---|---|
| キャリア・組織 | 13本 | 委譲テーマで1本追加余地あり（#15） |
| JavaScript/TypeScript | 11本 | 十分 |
| React/Next.js/フロントエンド | 12本 | 十分 |
| インフラ/Linux/Web基礎 | 16本 | 十分 |
| AWS/IaC/CI/CD | 13本 | CI自動テスト（#12）・ブランチ戦略（#13）で2本追加予定 |
| NestJS/バックエンド | 2本 | 余地あり。AI×バックエンド方向で |
| **AI/LangChain/Claude Code** | **8本** | **拡充優先。CLAUDE.md設計（#16）含め追加予定** |
| **医療IT/治験/規制対応** | **4本** | **拡充優先。4本追加予定（#6〜#9）** |
| チーム設計・マネジメント | - | タスク設計（#14）・委譲の恐怖（#15）を追加予定 |
| その他 | 1本 | |

---

## 🗂️ ポートフォリオ作品 追加計画

ポートフォリオページ（`/portfolios`）に載せる実装デモの計画。

### 優先度高（即戦力感を示す）

| # | タイトル | 概要 | 差別化ポイント |
|---|---|---|---|
| 1 | LangChain × NestJS AI連携デモ | 実務の自社CRMへのAI機能を個人プロジェクトとして再現 | LLM活用の実装力は現在最も差別化になる。実務ベースで説得力が段違い |
| 2 | Claude Code / AI開発フロー紹介 | ツール紹介ではなく「チームへの導入でレビュー工数をどう削減したか」のケーススタディ | 技術導入の判断・推進力を示せる。プロンプト設計まで踏み込む |

### 優先度中（技術幅を示す）

| # | タイトル | 概要 | 差別化ポイント |
|---|---|---|---|
| 3 | NestJS + TypeScript REST API | 認証・テスト込みのバックエンドAPI | フロントだけでなくバックエンドも書けることを示す |
| 4 | AWS CDK インフラ構成の解説 | このブログのインフラをコードと構成図で解説 | 現場エンジニアへの訴求力がある。IaC設計判断まで語れる |

> ブログ記事（`langchain-nestjs-ai-integration.md`, `cloudformation-to-cdk.md` など）との連携でポートフォリオ→ブログの動線を作る

---

## 📜 資格取得計画（取得後に記事化予定）

| 資格 | 分野 | 記事化メモ |
|---|---|---|
| AWS CLF（Cloud Practitioner） | AWS | 入口として。合格体験記よりAWS全体像の整理記事を狙う |
| AWS SAA（Solutions Architect Associate） | AWS | 検索流入が強い。「実務のどこで活きたか」セットで書く |
| AWS AIF（AI Foundations） | AWS / AI | AI記事との連携。LangChain・Claude Code との接続で差別化 |
| AWS MLS（Machine Learning Specialty） | AWS / AI | AIF取得後。ML実装記事と組み合わせる |
| 基本情報技術者 | IPA | 勉強ノート系が受ける。後輩エンジニア向けの解説記事にも転用 |
| 応用情報技術者 | IPA | 基本情報の次。マネジメント系記事との相乗効果 |
| マネジメント系（PM・PMP等） | マネジメント | テックリードのキャリア観と組み合わせると差別化になる |
| 簿記3級 / 2級 | 財務 | エンジニアが簿記を学んだ理由・IT投資の話との接続で独自性を出す |

> 取得したら `article-plan.md` の未執筆アイデアセクションに記事案を追記する。
> 勉強中に「こうやって勉強している」記事を書く方が、合格体験記より読まれやすいケースもある。

---

## 🏷️ タグ一覧（現在使用中）

```
技術系: TypeScript, JavaScript, React, Next.js, NestJS
       AWS, DevOps, インフラ, GitHub Actions
       LangChain, Claude Code, AI開発
       アーキテクチャ, パフォーマンス, データベース, セキュリティ

チーム系: チーム開発, テックリード, 組織設計, マネジメント

医療IT: 医療IT, 品質管理

キャリア: キャリア, 書評
```
計 24タグ（2026-05-24 に104タグから整理）
