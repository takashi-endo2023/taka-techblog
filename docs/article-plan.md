...
- **概要**: グループ会社（サプリEC）の立ち上げで、IT周りを丸ごと任された体験。ドメイン取得・レンタルサーバー契約・メール設定・WordPressコーポレート・MakeShop EC——何もないところから一式を作った。臨時ボーナスももらった
...
- **概要**: 入社前から放置されていたコーポレートサイト。HTMLファイルをレンタルサーバー（エックスサーバー）に直置きする保守性最悪のカオス状態だった。前期後半にWordPressで完全に作り直し、今季一発目に新サイトをオープンさせた実録
- **切り口**: 「カオスを引き継いで作り直す」あなたの最強パターン（AWS引き継ぎ/git-chaos/内製化の系譜）。負の遺産→リニューアルは共感を呼ぶ。エックスサーバー＋WordPressを実体験で語れる
- **配置アフィリ**: エックスサーバー（実際に運用中＝authentic）
