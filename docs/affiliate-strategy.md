...
| ConoHa WING | A8.net | 📝 提携予定 | 会社IT立ち上げ記事（初心者向け対比） |
| お名前.com | A8.net | 📝 提携予定 | 会社IT立ち上げ記事（ドメイン取得） |
...
### ConoHa WING / お名前.com（A8.net）— 会社IT立ち上げ系（📝提携予定）

**前提**: 配置先の記事（`company-launch-it-zero-to-one` / `startup-it-foundation-checklist`）は
**まだ未執筆**（article-plan のグループ会社シリーズ）。記事を書くときに配置する。

**お名前.com（ドメイン）**:
- 報酬110円・本人OK。AWS利用でもドメインは別途必要なので**矛盾しない**
- 「会社の立ち上げでドメインを取得した」文脈で自然

**ConoHa WING（レンタルサーバー）**:
- ⚠️ 自分はAWS(S3+CloudFront)構築。ConoHaを素で勧めると矛盾
- **「初心者はConoHaのようなレンタルサーバー、自分はAWSに移行した」という対比**でのみ使う
- 単独で「ConoHaおすすめ」とは書かない
