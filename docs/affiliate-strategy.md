...
| Audible | A8.net | 📝 提携予定 | 書評・学習法記事に「耳で聴く選択肢」 |
| ムームードメイン | A8.net | 📝 提携予定 | 会社IT立ち上げ記事（ドメイン取得・**実際に使った**） |
| ロリポップ | A8.net | 📝 提携予定 | 会社IT立ち上げ記事（サーバー・**実際に使った**） |
| エックスサーバー | A8.net | 📝 提携予定 | コーポレート刷新記事（WordPress運用・**実際に使った**） |
...
**配置対象**:
- 書評記事・career-change・学習法系に「耳で聴く選択肢」として一言
...
### ムームードメイン / ロリポップ（A8.net）— 会社IT立ち上げ系（📝提携予定）
...
- 「会社のコーポレート/ECはロリポップ＋WordPressで立ち上げた」実体験
- ⚠️ 個人ブログはAWS(Astro)だが**文脈が違うので矛盾しない**：
  「会社はロリポップ、個人ブログはAWSに移行した」と両方とも実体験として書ける。
  この対比がむしろ記事の深みになる

**配置タイミング**: 会社IT立ち上げ記事の執筆時。それまで提携だけして寝かせる。

---

### エックスサーバー（A8.net）— コーポレートサイト刷新（📝提携予定）

**配置先記事**: `corporate-site-renewal`（コーポレート刷新の体験談・未執筆）。

**実体験**: 入社前から放置されていたコーポレートサイト（HTMLファイルをエックスサーバーに
直置きする保守性最悪のカオス）を、WordPressで完全に作り直して本番運用中。
**実際に使い込んでいる**ので authentic に貼れる（個人ブログのAWSとは別案件・矛盾しない）。

**配置タイミング**: コーポレート刷新記事の執筆時。
