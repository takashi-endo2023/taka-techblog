...
> 「何を書くか・どう出すか・どこにアフィリを置くか」を見るときの1冊。
> 戦略・キャリアは `strategy.md`、毎日の実行は `status.md`、機械向けルールは `CLAUDE.md`。
...
## 関連ドキュメント
- `status.md` — 毎日〜毎週の実行盤　/　`post-log.md` — 日次1行ログ
- `strategy.md` — 北極星・X運用・マネタイズ・長期キャリア
