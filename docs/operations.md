...
| GEEKLY | A8 | テックリード・キャリア記事（GeeklyAffiliate）。**エージェント型** |
| リクルートダイレクトスカウト | A8 | ★**申請中（2026-06-30）／承認後に有効化**。ハイクラス・**スカウト型**（GEEKLY=エージェント型と別レーン＝同記事に並べても共倒れしない）。成果地点は会員登録。承認後 `RecruitDirectScout` コンポーネントを作成（GeeklyAffiliate と同型・PR表記/nofollow sponsored/トラッキングピクセル込み） |
...
| テックリード/組織/マネジメント | GEEKLY | リクルートダイレクトスカウト（承認後）※ |
| フロント/TS実務 | GEEKLY | Amazon |
| AWS/DevOps/インフラ実務 | Amazon（関連書） | リクルートダイレクトスカウト（承認後）※ |
| 学習・キャリア（未経験向け） | Udemy | — |
| 純粋な体験談・物語 | **なし**（物語性優先） | — |
| 医療IT/治験/規制 | なし（ニッチ・教材と乖離） | — |

- **未経験・学習中→Udemy**（GEEKLYは未経験NG）／**経験者・転職検討→GEEKLY**／**技術を深める→Amazon関連書**
- ※**リクルートダイレクトスカウト**（承認後）：GEEKLY と**別レーン**（エージェント型 vs スカウト型）。経験者向けキャリア記事で **GEEKLY と併用可**（1記事最大2の枠内）。「登録しておくと自分の市場価値=年収レンジが分かる」という**受け身でも価値がある**切り口で、市場感を掴む文脈に強い。未経験向けには出さない（ハイクラス対象のため）
