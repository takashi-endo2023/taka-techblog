...
## X/Zenn スタートダッシュ計画（2026年5月〜）

> X はアカウント開設直後。自己紹介固定ポスト済み。
> **最初の印象が全て** → 一番強い記事から告知し「AIとテックリードの人」を刷り込む。

### 5/25（日）完了 ✓

- [x] Zenn: #3「mainブランチを捨てた」公開済み
- [x] X: git-chaos 記事告知 投稿済み

### 5/26（月）完了 ✓

- [x] X: #1「島流しの先にエンジニアがあった」記事告知 投稿済み

### 5/27（火）完了 ✓

- [x] GEEKLY（A8.net）承認 → アフィリエイト実装
- [x] `GeeklyAffiliate.astro` コンポーネント新規（banner/cta/inline 3バリアント）
- [x] テックリード/組織系9本 + フロントエンド/TS実務系5本 + 既実装1本 = **計15記事にアフィリ配置**
- [x] CloudFront CSP の `img-src` に `https://*.a8.net` 追加（CDK経由でデプロイ）
- [x] `sync-zenn.js` に GeeklyAffiliate / PRセクション除去ルール追加
- [x] privacy.astro の GEEKLY 運営会社名修正（株式会社ギークリー）
- [x] Zenn: #52「TypeScriptとReact/Next.js実践本——書評（テックリード視点）」公開
...
| 日付 | アクション | 内容 |
|------|-----------|------|
| ~~5/26 月~~ | ~~**X 記事告知** #1「島流しの先にエンジニアがあった」~~ | ✅ 完了 |
...
遠回りだったけど、あの異動がなければ今の自分はない。
転身の現実を書きました →
https://www.taka-techblog.com/blog/career-change-to-engineer
...
チームに見せる自分と、実際の自分のギャップを
3年間ずっと抱えていた。全部書きました →
https://www.taka-techblog.com/blog/techlead-mask-3years
...
### 再来週以降（6/8〜）の X 告知優先リスト

週2本ペースで上から消化する。

| 優先 | 記事 | ブログURL末尾 |
|------|------|-------------|
| 1 | テックリード就任最初の90日 | `techLead-first-90-days` |
| 2 | Claude Code実務導入 | `claude-code-workflow` |
| 3 | 30歳からの転身の現実 | `career-change-to-engineer` |
| 4 | Cursor実務レビュー2025 | `cursor-review-2025` |
| 5 | Claude Codeを設計の壁打ち相手に | `claude-code-as-design-partner` |
| 6 | プレイングマネージャーの現実 | `playing-manager-reality` |
| 7 | コードレビューを3年一人でやった話 | `code-review-culture` |
| 8 | 未経験エンジニア2名をコーチング | `coaching-junior-engineers` |
| 9 | 経営層への技術翻訳 | `communicating-it-value-to-executives` |
| 10 | 技術的負債と向き合う3年間 | `technical-debt-strategy` |
