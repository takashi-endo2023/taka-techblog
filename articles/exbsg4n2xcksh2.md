...
S3 + CloudFrontの構成を構築したあとは、ファイル種別ごとのキャッシュ設定を最適化することでサイトのパフォーマンスをさらに引き上げられる。Astroのビルドアーティファクトに合わせたキャッシュ戦略の詳細は[AstroサイトのS3+CloudFrontキャッシュ戦略：静的サイトを最速にする](/blog/astro-s3-cache-strategy)でまとめている。
