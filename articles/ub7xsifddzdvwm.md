...
:::message
この記事は [taka-techblog](https://www.taka-techblog.com/blog/astro-s3-cache-strategy?utm_source=zenn&utm_medium=referral) にも掲載しています。
:::
...
- S3へのデプロイ順序: `_astro/` →HTMLの順で
- CloudFront Invalidationの `/*` 指定は1パスカウント
- CDKでキャッシュポリシーをコード管理するとパスごとに細かく設定できる

このキャッシュ戦略を実装しているブログのインフラ全体——S3バケット・CloudFrontディストリビューション・Route53・GitHub ActionsによるCI-CDの構成については、[AstroとAWS CDK + CloudFrontで技術ブログを構築した話](/blog/astro-aws-cdk-cloudfront-blog)でまとめている。
